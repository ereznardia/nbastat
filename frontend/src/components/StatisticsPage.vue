  ...
  methods: {
    async fetchStats() {
      ...
      const playerFetches = this.players.map(async player => {
        const url = `http://localhost:8080/api/${this.selectedYear}/player/${player.player_id}/${this.selectedStat}`;
        try {
      ...
      const teamFetches = this.teams.map(async team => {
        const url = `http://localhost:8080/api/${this.selectedYear}/team/${team.team_id}/${this.selectedStat}`;
        try {
          const res = await fetch(url);
          const data = await res.json();
